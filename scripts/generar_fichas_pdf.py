...
def main():
    ...
    ap.add_argument('--lista', default='',
                    help='archivo de texto con un código de ficha por línea; '
                         'genera solo esas')
    ...
    filtro_codigo = set()
    if args.lista:
        with io.open(args.lista, encoding='utf-8') as f:
            filtro_codigo = {l.strip() for l in f if l.strip()}
        print(f'Lista de códigos: {len(filtro_codigo)}')
    ...
    lote = [p for p in fichas
            ...
            and (not filtro_codigo or (p.get('_codigo') or '') in filtro_codigo)]
